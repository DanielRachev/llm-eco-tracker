...
from scheduler import _get_live_delay_seconds
...
def carbon_aware(max_delay_hours=2, location="NL", mock_csv=None):
    ...
    def _get_delay_seconds():
        if mock_csv:
            logger.info("Mock mode enabled: reading from CSV...")
            # TODO: Implement reading from mock_data.csv here for the experiment
            return 5
        else:
            logger.info("Live mode enabled: fetching UK Grid forecast...")
            return _get_live_delay_seconds(max_delay_hours)
